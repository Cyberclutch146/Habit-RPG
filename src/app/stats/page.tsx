    ...
    <>
      ...
      <main className="pt-20 pb-32 px-4 w-full flex-1 overflow-y-auto space-y-6 custom-scrollbar">
        ...
        <section className="bg-surface-container rounded-xl shadow-xl border border-outline-variant/10 relative overflow-hidden h-40 flex items-center justify-center">
            ...
            <RanksManual isOpen={isRanksManualOpen} onClose={() => setIsRanksManualOpen(false)} />
        </section>
        ...
        {activeTab === "GEAR" && (
            <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
                ...
                {/* Pets Companion Section */}
                <div className="bg-surface-container-high p-4 rounded-xl border border-outline-variant/10 shadow-xl">
                    <h2 className="text-sm font-black text-on-surface tracking-tighter uppercase flex items-center gap-2 mb-4">
                        <span className="material-symbols-outlined text-amber-400">pets</span>
                        Companion
                    </h2>
                    
                    {(() => {
                      const equippedPet = PET_ROSTER.find(p => p.id === user?.equippedPet);
                      const ownedCount = (user?.pets || []).length;
                      ...
                      if (equippedPet) {
                        ...
                        const rc = rarityColors[equippedPet.rarity];
                        
                        return (
                          <div className={`flex items-center gap-4 p-3 rounded-xl border-2 ${rc}`}>
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${rc}`}>
                              <span className="material-symbols-outlined text-2xl">{equippedPet.icon}</span>
                            </div>
                            <div className="flex-1">
                              <p className="font-bold text-on-surface text-sm">{equippedPet.name}</p>
                              <p className="text-[10px] text-secondary">{equippedPet.description}</p>
                            </div>
                            <span className="text-[9px] uppercase font-bold tracking-widest text-primary bg-primary/10 px-2 py-1 rounded">
                              Active
                            </span>
                          </div>
                        );
                      ...
                      return (
                        <div className="text-center py-4">
                          ...
                          <p className="text-xs text-secondary font-bold uppercase tracking-widest">No companion equipped</p>
                          <p className="text-[10px] text-secondary/60 mt-1">{ownedCount > 0 ? "Tap below to equip one!" : "Earn pets through streak milestones"}</p>
                        </div>
                      );
                    })()}

                    <button
                      onClick={() => { playClick(); setIsPetCollectionOpen(true); }}
                      className="w-full mt-3 py-2.5 text-[10px] uppercase font-bold tracking-widest text-primary bg-primary/5 hover:bg-primary/10 border border-primary/20 rounded-xl transition-colors flex items-center justify-center gap-2"
                    >
                      <span className="material-symbols-outlined text-sm">collections</span>
                      View Collection ({(user?.pets || []).length}/{PET_ROSTER.length})
                    </button>
                </div>
